...
| Resources/state | **NOT VERIFIED** | Exact app name `Demo_App_HR` (UI title, docs); local-Docker cgroup limits (cpu.max 50000/100000, memory.max 1 GiB, swap 0, zero mounts, uid 1000: spike S5, `_agents/projects/HR/spikes/S5-readonly-fs.md`) | The §8 20-minute workload and peak-memory gate: NOT VERIFIED; only the 5-minute smoke in `RESOURCE_TESTS.md` was run. Clean redeploy and state recovery: evidence is the slice-5 re-launch report, not this file. WAR 0.5-core/1-GiB/zero-volume deployment: OUT OF SCOPE (D3) |
| Quality | **NOT VERIFIED** | `npm audit --omit=dev` and the CycloneDX SBOM (`RESOURCE_TESTS.md`, "Dependency audit"; `sbom.cdx.json`); all critical and important review findings fixed (table above); lower-severity findings listed in `DEFERRED.md` | Image, secret and SAST scans and authenticated DAST: NOT VERIFIED (D8). CI-based scanning: OUT OF SCOPE (D4) |
