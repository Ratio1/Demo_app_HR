...
describe("manage migrate", () => {
  it("applies 0001_init to an empty database", async () => {
    ...
    expect(rows.map((row) => row.id)).toEqual(["0001_init"]);
  });
  ...
  it("is a no-op when run again", async () => {
    const before = await withClient(ownerPool, async (client) =>
      (await client.query<{ id: string; applied_at: Date }>("SELECT id, applied_at FROM schema_migrations")).rows,
    );
    const result = await runMigrations(ownerPool, { appRole });
    expect(result.applied).toEqual([]);
    expect(result.alreadyApplied).toEqual(["0001_init"]);
  ...
  it("grants the runtime role exactly INSERT and SELECT on audit_events", async () => {
    const rows = await withClient(ownerPool, async (client) =>
      (
    ...
    for (const table of ["accounts", "sessions", "settings", "employees", "leave_requests"]) {
      expect(held(table)).toEqual(["DELETE", "INSERT", "SELECT", "UPDATE"]);
    }
  });
});
...
describe("the runtime role in practice", () => {
  ...
  it("cannot write the migration journal but can read it", async () => {
    await withClient(appPool, async (client) => {
      ...
      const journal = await client.query<{ id: string }>("SELECT id FROM schema_migrations");
      expect(journal.rows.map((row) => row.id)).toEqual(["0001_init"]);
    });
  });

  it("can read and write the business tables (the control for the refusals above)", async () => {
    ...
    await withClient(appPool, async (client) => {
      ...
      const removed = await client.query("DELETE FROM accounts WHERE id = $1", [accountId]);
