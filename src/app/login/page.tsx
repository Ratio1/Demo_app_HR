 ...
 * generic copy per flows.md S1's `valid` row. **`429` is answered directly by the route as its
 * own small HTML page** (slice-4 "App defects found" #5's fix — no redirect, but no raw JSON
 * body either, since `LoginForm` is a plain, script-free form that navigates straight to it).
 * **`403`** (missing/mismatched Origin or CSRF) is still a JSON problem body with no redirect —
 * unchanged, and this page cannot intercept or restyle it either. `?email=` is read defensively
 * in case a later change starts sending the typed address back, but the current route never
 * does — flows.md's "preserved: the typed email address" rule does not yet hold across this
 * redirect, flagged in the report rather than silently assumed to work.
 */
